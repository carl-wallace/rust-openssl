...
foreign_type_and_impl_send_sync! {
    type CType = ffi_10_55::OSSL_PROVIDER;
...
#[inline]
unsafe fn ossl_provider_free(p: *mut ffi_10_55::OSSL_PROVIDER) {
    ffi_10_55::OSSL_PROVIDER_unload(p);
}
...
impl Provider {
    ...
    pub fn load(ctx: Option<&LibCtxRef>, name: &str) -> Result<Self, ErrorStack> {
        ...
        unsafe {
            let p = cvt_p(ffi_10_55::OSSL_PROVIDER_load(
    ...
    pub fn try_load(
        ...
        let name = CString::new(name).unwrap();
        unsafe {
            let p = cvt_p(ffi_10_55::OSSL_PROVIDER_try_load(
                ctx.map_or(ptr::null_mut(), ForeignTypeRef::as_ptr),
                name.as_ptr(),
    ...
    pub fn set_default_search_path(ctx: Option<&LibCtxRef>, path: &str) -> Result<(), ErrorStack> {
        ...
        unsafe {
            cvt(ffi_10_55::OSSL_PROVIDER_set_default_search_path(
                ctx.map_or(ptr::null_mut(), ForeignTypeRef::as_ptr),
                path.as_ptr(),
            ))
            .map(|_| ())
